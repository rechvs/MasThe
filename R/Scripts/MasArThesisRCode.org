...
* StandAgeSIYieldClassClassification.pdf
  ...
  #+BEGIN_SRC R :results silent :var input=Preamble :session *MasArThesisRConsole*
	   ...
	   panel.first = abline(v = seq(from = kXLim[1],
        ...
        legend.legend <- c(legend.legend,
		       mapply(FUN = function(expr, numbr) { as.expression(x = bquote(.(expr)*.(numbr))) },  ## See https://stackoverflow.com/questions/27275798/combining-vector-variables-in-r-expression-for-plot-text?noredirect=1.
			    ...
			    numbr = SI.h100.yield.class.values[index.worst.yield.class.needed : index.best.yield.class.needed, "yield.class"]))
    ...
    rm(list = setdiff(x = ls(), y = objects.at.script.start))
  #+END_SRC
* SiteIndexRangeSPECIES.txt
  #+NAME: SiteIndexRangeSPECIES.txt
  #+BEGIN_SRC R :results silent :var input=Preamble :session *MasArThesisRConsole*
    ## TEX file: ~/laptop02_MasAr/LaTeX/Thesis/DataDescription.tex
    ## Loop over all species names.
    for (cur.species.name in kSpeciesNames) {
        ...
        cur.data.frame <- get(x = paste0("bart.", cur.species.name, ".clean.1.8"))
        ## Extract top height range.
        SI.h100.min <- min(cur.data.frame[["SI.h100"]])
        SI.h100.max <- max(cur.data.frame[["SI.h100"]])
        ## Store results in data frame.
        cur.species.results.df <- data.frame("statistic" = c("min", "max"),
				     "SI.h100" = c(SI.h100.min, SI.h100.max))
        ## Store printing of "cur.species.results.df" in "cur.output", while left justifying output.
        cur.output <- capture.output(print(x = format(x = cur.species.results.df,
					    justify="left",
					    scientific = FALSE),
        ...
        ## Output results to file.
        cat(cur.output,
	  file = paste0(kOutputSubdir, "SiteIndexRange", cur.species.name.capitalized, ".txt"),
	  sep = "\n",
	  fill = FALSE)
    }
    ## Clean up workspace.
...
* VariablesStatisticsSPECIES.txt
  ...
  #+BEGIN_SRC R :results silent :var input=Preamble :session *MasArThesisRConsole*
    ...
    for (cur.species.name in kSpeciesNames) {
        ...
        ## Get data frame for current species.
        cur.data.frame <- get(x = paste0("bart.", cur.species.name, ".clean.1.8"))
        ## Initiate object in which to store results for current species.
        ...
        colnames(x = cur.species.results.df) <- c("statistic", kVariablesColumnNames)
        cur.species.results.df[["statistic"]] <- kStatisticFunctionsNames
	  ...
	  for (cur.stat.fun.name in kStatisticFunctionsNames) {
	      ...
	      ## Calclulate current statistic and store it in the appropriate row and column of "cur.species.results.df".
	      cur.species.results.df[cur.species.results.df[["statistic"]] == cur.stat.fun.name, cur.col.name] <- cur.stat.fun(x = cur.data.frame[[cur.col.name]],
												         na.rm = TRUE)
	  }}
        ...
        ## Output results to file.
        cat(cur.output,
	  file = paste0(kOutputSubdir, "StatisticsOfVariablesSPECIES", cur.species.name.capitalized, ".txt"),
	  sep = "\n",
	  fill = FALSE)
    }
    ## Clean up workspace.
