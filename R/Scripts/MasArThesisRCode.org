    ...
    for (cur.species.name in kSpeciesNames) {
	  ...
	  SI.h100.yield.class.values[["SI.h100"]][SI.h100.yield.class.values[["yield.class"]] == cur.yield.class] <- SI.h100.yield.class.values[["SI.h100"]][SI.h100.yield.class.values[["yield.class"]] == cur.yield.class + 1] + SI.h100.yield.class.1 - SI.h100.yield.class.2
	   ...
	   panel.first = abline(v = seq(from = kXLim[1],
        ...
        ## Update legend components.
        legend.legend <- c(legend.legend,
		       mapply(FUN = function(expr1, expr2, expr3) { as.expression(x = bquote(.(expr1)[.(expr2)]*" ("*.(expr3)*" m)")) },  ## See https://stackoverflow.com/questions/27275798/combining-vector-variables-in-r-expression-for-plot-text?noredirect=1.
			    expr1 = expression(italic("PI")),
			    expr2 = paste0("YC ", SI.h100.yield.class.values[index.worst.yield.class.needed : index.best.yield.class.needed, "yield.class"]),
			    expr3 = SI.h100.yield.class.values[["SI.h100"]][index.worst.yield.class.needed:index.best.yield.class.needed])
    )
  ...
  #+BEGIN_SRC R :results silent :var input=Preamble :session *MasArThesisRConsole*
    ...
    for (cur.species.name in kSpeciesNames) {
	  ...
	  SI.h100.yield.class.values[["SI.h100"]][SI.h100.yield.class.values[["yield.class"]] == cur.yield.class] <- SI.h100.yield.class.values[["SI.h100"]][SI.h100.yield.class.values[["yield.class"]] == cur.yield.class + 1] + SI.h100.yield.class.1 - SI.h100.yield.class.2
	   ...
	   ylim = kYLim,
	   xlab = "Stand age [a]",
	   ylab = expression(italic("h")[100]*"("*italic("x")*")"["YC 1"]*" [m]"),
	   main = kSpeciesNamesCapitalized[[cur.species.name]],
	   mgp = kPlotMgp,
	   xaxs = "i",
	   yaxs = "i",
...
** GAM1EffectStandAgeVariable.pdf
   ...
   #+BEGIN_SRC R :results silent :var input=GAM1Preamble :session *MasArThesisRConsole*
     ...
     pdf(file = file.name,
         width = kPdfWidthA4Portrait,
         height = kPdfHeightA4Portrait * 0.94,
         pointsize = kPdfPointSize,
         family = kPdfFamily)
     ## Set plot layout.
     layout(mat = kPlotLayout)
...
** GAM1EffectProductivityIndexVariable.pdf
   ...
   #+BEGIN_SRC R :results silent :var input=GAM1Preamble :session *MasArThesisRConsole*
     ...
     pdf(file = file.name,
         width = kPdfWidthA4Portrait,
         height = kPdfHeightA4Portrait * 0.92,
         pointsize = kPdfPointSize,
         family = kPdfFamily)
     ## Set plot layout.
     layout(mat = kPlotLayout)
...
** GAM1FittedValuesResiduals.pdf
   ...
   #+BEGIN_SRC R :results silent :var input=GAM1Preamble :session *MasArThesisRConsole*
     ...
     pdf(file = file.name,
         width = kPdfWidthA4Portrait,
         height = kPdfHeightA4Portrait * 0.97,
         pointsize = kPdfPointSize,
         family = kPdfFamily)
     ## Set plot layout.
     layout(mat = kPlotLayout)
...
** GAMLSS2QQPlot.pdf
   ...
   #+BEGIN_SRC R :results silent :var input=GAMLSS2Preamble :session *MasArThesisRConsole*
     ...
     pdf(file = file.name,
         width = kPdfWidthA4Portrait,
         height = kPdfHeightA4Portrait * 0.905,
         pointsize = kPdfPointSize,
         family = kPdfFamily)
     ## Set plot layout.
     layout(mat = kPlotLayout)
...
* TopHeightBasalAreaObservationsGAMLSS3SCAM1SterbaPredictionsYieldClassClassification.pdf
  ...
  #+BEGIN_SRC R :results silent :var input=Preamble :session *MasArThesisRConsole*
    ...
    kSterbaCoefs <- list("Döbbeler" = list("beech" = list("a0" = 1.0829E-07,  ## Source: Döbbeler (2004), tab. 4.2, Region Nordwest
					        "a1" = 1.5374,
					        "b0" = 8.3652,
					        "b1" = -1.7365),
				   "spruce" = list("a0" = 1.28745E-06,
					         ...
					         "b0" = 1.2842,
					         "b1" = -1.1914)),
		     "Wördehoff" = list("beech" = list("a0" = 2.616551e-07,  ## Source: Wördehoff (2016), tab. 3.6
					         "a1" = 1.368151,
					         "b0" = 6.496417,
					         "b1" = -1.731867),
    ...
    pdf(file = kFilename,
        width = kPdfWidthA4Portrait,
        height = kPdfHeightA4Portrait * 0.82,
        pointsize = kPdfPointSize,
        family = kPdfFamily)
    ## Set plot layout.
    layout(mat = kPlotLayout)
    ...
    for (cur.species.name in kSpeciesNames) {
	  ...
	  ## Update legend components.
	  legend.legend <- c(legend.legend,
			 paste0(cur.coefs.source.name, " coeff. ", cur.species.name))
	  legend.col <- c(legend.col,
    ...
    rm(list = setdiff(x = ls(), y = objects.at.script.start))
  #+END_SRC
* TopHeightBasalAreaSchober.pdf
  #+NAME: TopHeightBasalAreaSchober.pdf
  #+BEGIN_SRC R :results silent :var input=Preamble :session *MasArThesisRConsole*
    ## TEX file: ~/laptop02_MasAr/LaTeX/Thesis/DiscussionModelsOverview.tex
    ## Plotting preamble.
    kPlotMargins <- c(3, 3.25, 2, 1)
    ...
    kBeechSchober <- data.frame("yield.class" = c(rep(x = 1,
    ...
    kSpruceSchober <- data.frame("yield.class" = c(rep(x = 1,
    ...
    for (cur.species.name.capitalized in kSpeciesNamesCapitalized) {
        ...
        cur.yield.table.df <- get(x = paste0("k", cur.species.name.capitalized, "Schober"))
        ## Set plot margins.
        par("mar" = kPlotMargins)
        ## Create empty plot.
    ...
    rm(list = setdiff(x = ls(), y = objects.at.script.start))
  #+END_SRC
* StandAgeBasalAreaSchober.pdf
  #+NAME: StandAgeBasalAreaSchober.pdf
  #+BEGIN_SRC R :results silent :var input=Preamble :session *MasArThesisRConsole*
    ## TEX file: ~/laptop02_MasAr/LaTeX/Thesis/DiscussionModelsOverview.tex
    ## Plotting preamble.
    kPlotMargins <- c(3, 3.25, 2, 1)
    ...
    rm(list = setdiff(x = ls(), y = objects.at.script.start))
  #+END_SRC
* StandAgeTopHeightSchober.pdf
  #+NAME: StandAgeTopHeightSchober.pdf
  #+BEGIN_SRC R :results silent :var input=Preamble :session *MasArThesisRConsole*
    ## TEX file: ~/laptop02_MasAr/LaTeX/Thesis/DiscussionModelsOverview.tex
    ## Plotting preamble.
    kPlotMargins <- c(3, 3.25, 2, 1)
    ...
    kGridIntervalH <- 5
    kXLab <- "Stand age [a]"
    kYLab <- expression(italic("h")["100"]*" [m]")
    kLineCols <- list("1" = "royalblue",
		  "2" = "green",
		  "3" = "orange",
		  "4" = "red")
    ...
    for (cur.species.name.capitalized in kSpeciesNamesCapitalized) {
	  ...
	  ## Add lines to plot.
	  lines(x = cur.yield.table.df[["stand.age"]][cur.yield.table.df[["yield.class"]] == cur.yield.class.index],
	        y = cur.yield.table.df[["top.height"]][cur.yield.table.df[["yield.class"]] == cur.yield.class.index],
	        col = kLineCols[[cur.yield.class.index]],
	        lty = kLineType,
	        lwd = kLineWidth)
	  ## Add points to plot.
    ...
    rm(list = setdiff(x = ls(), y = objects.at.script.start))
  #+END_SRC
* StandAgeTopHeightTestData.pdf
  #+NAME: StandAgeTopHeightTestData.pdf
  #+BEGIN_SRC R :results silent :var input=Preamble :session *MasArThesisRConsole*
    ## TEX file: ~/laptop02_MasAr/LaTeX/Thesis/DiscussionModelsOverview.tex
    ## Plotting preamble ##
    kPlotMargins <- c(3, 3.25, 2, 1)
    ...
    kGridIntervalV <- 10
    kGridIntervalH <- 10
    kLegendX <- list("beech" = "bottomright",
		 "spruce" = "bottomright")
    kXValuesColumnNameTestData <- "age"
    kYValuesColumnNameTestData <- "h100"
    kXLab <- "Stand age [a]"
    kYLab <- expression(italic("h")["100"]*" [m]")
    ## Create file name.
    kFilename <-paste0(kGraphicsSubdir, "StandAgeTopHeightTestData.pdf")
    ## Define color palette generating function.
    kColorPaletteFunc <- colorRampPalette(colors = c("red", "yellow", "springgreen", "royalblue"))
    ## Set best yield class level for which to interpolate SI.h100 values.
    kBestYieldClassLevel <- -2
    ...
    pdf(file = kFilename,
        width = kPdfWidthA4Portrait,
        height = kPdfHeightA4Portrait * 0.91,
        pointsize = kPdfPointSize,
        family = kPdfFamily)
    ## Set plot layout.
    layout(mat = kPlotLayout)
    ...
    for (cur.species.name in kSpeciesNames) {
	  ...
	  test.df <- rbind(test.df,
        ...
        par("mar" = kPlotMargins)
        ## Create empty plot.
        plot(x = test.df[[kXValuesColumnNameTestData]],
	   type = "n",
	   xlim = kXLim,
	   ylim = kYLim,
	   xlab = kXLab,
	   ...
	   panel.first = abline(v = seq(from = kXLim[1],
        ...
        ## Add lines to plot per yield class.
        all.cols <- cur.species.color.palette[SI.h100.yield.class.values.color.index]
        for (cur.yield.class.index in seq_len(length.out = length(x = levels(x = test.df[["yield.class"]])))) {
	  yield.class.name <- levels(x = test.df[["yield.class"]])[cur.yield.class.index]
	  line.col <- all.cols[cur.yield.class.index]
	  lines(x = test.df[[kXValuesColumnNameTestData]][test.df[["yield.class"]] == yield.class.name],
	        y = test.df[[kYValuesColumnNameTestData]][test.df[["yield.class"]] == yield.class.name],
	        col = line.col,
	        lty = kLineType,
	        lwd = kLineWidth)
        }
    ...
    rm(list = setdiff(x = ls(), y = objects.at.script.start))
  #+END_SRC
* TopHeightBasalAreaObservationsGAMLSS3SCAM1WoerdehoffPredictionsYieldClassClassification.pdf
  #+NAME: TopHeightBasalAreaObservationsGAMLSS3SCAM1WoerdehoffPredictionsYieldClassClassification.pdf
  #+BEGIN_SRC R :results silent :var input=Preamble :session *MasArThesisRConsole*
    ## TEX file: ~/laptop02_MasAr/LaTeX/Thesis/DiscussionModelsOverview.tex
    ## Modelling preamble.
    library(package = "scam", character.only = TRUE)
    ...
    kYValuesColumnNameTrainingData <- "gha"
    kYValuesColumnNameTestData <- "gha.predictions"
    kWoerdehoffIndependentVariableName <- "h100"
    kWoerdehoffFormula <- as.formula(object = paste0(kDependentVariableRName, " ~ ", "ps(", kWoerdehoffIndependentVariableName, ")"))
    kWoerdehoffDistFamily <- BCCG()
    kWoerdehoffTestData <- data.frame(seq(from = 0,
				  to = 100,
				  by = 0.1))
    names(x = kWoerdehoffTestData) <- c(kWoerdehoffIndependentVariableName)
    ## Plotting preamble ##
    kPlotMargins <- c(3, 3.25, 2, 1)
    kPlotMgp <- c(2, 1, 0)
    kPlotLayout <- matrix(data = 1:2,
    ...
    kXLab <- expression(italic("h")["100"]*" [m]")
    kYLab <- expression(italic("G")*" ["*m^2*" ha"^-1*"]")
    kWoerdehoffLineCol <- "black"
    kWoerdehoffLineType <- "longdash"
    ## Create file name.
    kFilename <-paste0(kGraphicsSubdir,
		   "TopHeightBasalAreaObservationsGAMLSS3SCAM1WoerdehoffPredictionsYieldClassClassification.pdf")
    ## Define color palette generating function.
    ...
    pdf(file = kFilename,
        width = kPdfWidthA4Portrait,
        height = kPdfHeightA4Portrait * 0.86,
        pointsize = kPdfPointSize,
        family = kPdfFamily)
    ## Set plot layout.
    layout(mat = kPlotLayout)
    ...
    for (cur.species.name in kSpeciesNames) {
        ## Get capitalized name for current species.
        cur.species.name.capitalized <- kSpeciesNamesCapitalized[[cur.species.name]]
        ## Get Wördehoff test data.
        cur.Woerdehoff.test.data <- kWoerdehoffTestData
        ## Get input data frame for current species.
        cur.input.data <- get(x = paste0("bart.", cur.species.name, ".clean.1.8"))
        ## Generate test data ##
        ## The test data should cover the same yield class range as the training data.
	  ...
	  lines(x = new.data[[kXValuesColumnNameTestData]][new.data[["yield.class"]] == yield.class.name],
        ...
        legend.lwd <- c(legend.lwd,
		    rep(x = kLineWidth,
		        times = length(x = SI.h100.yield.class.values[index.worst.yield.class.needed : index.best.yield.class.needed, "yield.class"])))
        ## Add Wördehoff predictions to plot ##
        ## Subset input data frame to the variables mentioned in the model formula.
        cur.input.data.col.subset <- cur.input.data[, all.vars(expr = kWoerdehoffFormula)]
        ## Remove missing values from "cur.input.data.col.subset".
        cur.input.data.col.subset.na.omitted <- na.omit(object = cur.input.data.col.subset)
        ## Fit model.
        Woerdehoff.model <- gamlss(formula = kWoerdehoffFormula,
			     sigma.formula = kWoerdehoffFormula,
			     nu.formula = kWoerdehoffFormula,
			     data = cur.input.data.col.subset.na.omitted,
			     family = kWoerdehoffDistFamily,
			     method = RS(1000))
        ## Calculate model predictions.
        cur.Woerdehoff.test.data[[kYValuesColumnNameTestData]] <- predict(object = Woerdehoff.model,
							    newdata = cur.Woerdehoff.test.data,
							    what = "mu",
							    type = "response")
        ## Add predictions to plot.
        lines(x = cur.Woerdehoff.test.data[[kWoerdehoffIndependentVariableName]],
	    y = cur.Woerdehoff.test.data[[kYValuesColumnNameTestData]],
	    col = kWoerdehoffLineCol,
	    lty = kWoerdehoffLineType,
	    lwd = kLineWidth)
        ## Update legend components.
        legend.legend <- c(legend.legend,
		       "Wördehoff")
    ...
    rm(list = setdiff(x = ls(), y = objects.at.script.start))
  #+END_SRC
* ModelsStatistics.txt
  #+NAME: ModelsStatistics.txt
  #+BEGIN_SRC R :results silent :var input=Preamble :session *MasArThesisRConsole*
    ## TEX file:
    ## Modelling preamble.
    for (pckg in c("mgcv", "scam", "gamlss", "gamlss.dist")) {
    ...
    kTauFormula <- kSigmaFormula
    ## Preamble.
    kDestFilename <- paste0(kOutputSubdir,
		    "ModelsStatistics.txt")
    ## Initiate list in which to store results.
    inner.list <- vector(mode = "list", length = length(x = kSpeciesNames))
    names(x = inner.list) <- kSpeciesNamesCapitalized
    res.list <- vector(mode = "list", length = length(x = kFormulas))
    names(x = res.list) <- names(x = kFormulas)
    for (cur.index in seq_len(length.out = length(x = res.list))) {
        res.list[[cur.index]] <- inner.list
    }
    ## Loop over all formula names.
    for (cur.formula.name in names(x = kFormulas)) {
	  ...
	  cur.data.frame <- na.omit(object = subset(x = get(x = paste0("bart.", cur.species.name, ".clean.1.8")),
					    select = c(kDependentVariableRName,
						     kStandAgeVariableRName,
						     kProductivityIndexVariableRName)))
	  ## Fit model, using the appropriate settings and fitting function.
	  if (grepl(x = cur.formula.name, pattern = "GAM[12]")) {
	  cur.model <- gam(formula = cur.formula,
		         data = cur.data.frame,
		         family = Gamma(link = "log"),
		         na.action = na.omit)
	  }
	  if (cur.formula.name == "SCAM1") {
	   cur.model <- scam(formula = cur.formula,
			 data = cur.data.frame,
			 family = Gamma(link = "log"))
	  }
	  if (grepl(x = cur.formula.name, pattern = "GAMLSS[123]")) {
	      cur.model <- gamlss(formula = cur.formula,
			      sigma.formula = kSigmaFormula,
			      nu.formula = kNuFormula,
			      tau.formula = kTauFormula,
	  ...
	  ## Store summary of current model at the appropriate location in "res.list".
	  res.list[[cur.formula.name]][[cur.species.name.capitalized]] <- summary(object = cur.model)
        }}
    ## Store printing of "res.list" in "output", while left justifying output.
    output <- capture.output(print(x = res.list))
    ## Output results to file.
    cat(output,
        file = kDestFilename,
        sep = "\n",
