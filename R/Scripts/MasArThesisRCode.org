...
* AIC.txt
  ...
  #+BEGIN_SRC R :results silent :var input=Preamble :session *MasArThesisRConsole*
    ...
    rm(list = setdiff(x = ls(), y = objects.at.script.start))
  #+END_SRC
* TopHeightBasalAreaObservationsSterbaPredictions.pdf
  #+NAME: TopHeightBasalAreaObservationsSterbaPredictions.pdf
  #+BEGIN_SRC R :results silent :var input=Preamble :session *MasArThesisRConsole*
    ## TEX file: ~/laptop02_MasAr/LaTeX/Thesis/Discussion.tex
    ## Modelling preamble.
    library(package = "nls2", character.only = TRUE)
    library(package = "nlme", character.only = TRUE)
    kDepVarsColNames <- c("dg", "nha", "gha")
    kSterbaFormulas <- list(
        "dg" = as.formula(object = "dg ~ 1 / (coef.a0 * h100 ^ coef.a1 * nha + coef.b0 * h100 ^ coef.b1)"),
        "nha" = as.formula(object = "nha ~ coef.b0 / coef.a0 * (2 * coef.b0 * dg) ^ ((coef.a1 / coef.b1) - 1)"),
        "gha" = as.formula(object = "gha ~ pi / (16 * coef.a0 * coef.b0 * h100 ^ (coef.a1 + coef.b1))"))
    kStartValsVecs <- list("dg" = list(
			 "beech" = c("coef.a0" = 1.0829 * 10 ^ -7,  ## source: Döbbeler (2004), tab. 4.2
				   "coef.a1" = 1.5374,
				   "coef.b0" = 8.3652,
				   "coef.b1" = -1.7365),
			 "spruce" = c("coef.a0" = 1.28745 * 10 ^ -6,  ## source: Döbbeler (2004), tab. 4.2
				    "coef.a1" = 0.7148,
				    "coef.b0" = 1.2842,
				    "coef.b1" = -1.1914)),
		       "nha" = list(
			 ...
			 "spruce" = c("coef.a0" = 1.28745 * 10 ^ -6,  ## source: Döbbeler (2004), tab. 4.2
				    ...
				    "coef.b1" = -1.1914)),
		       "gha" = list(
			 "beech" = c("coef.a0" = 2.616551 * 10 ^ -7,  ## source: Wördehoff (2016), tab. 3.6
				   "coef.a1" = 1.368151,
				   "coef.b0" = 6.496417,
				   "coef.b1" = -1.731867),
			 "spruce" = c("coef.a0" = 4.913256 * 10 ^ -6,  ## source: Wördehoff (2016), tab. 3.6
				    "coef.a1" = 0.4394706,
				    "coef.b0" = 0.3716977,
				    "coef.b1" = -0.9097641)))
    kSterbaCoefs <- list("beech" = list("a0" = 2.616551e-07,
				"a1" = 1.368151,
				"b0" = 6.496417,
				"b1" = -1.731867),
    ...
    for (cur.species.name in kSpeciesNames) {
	  ...
	  points(x = edvid.subset[[kXValuesColumnNameTrainingData]],
        ...
        for (cur.depvar.colname in kDepVarsColNames[3]) {
	  ## Get formula for current dependent variable.
	  cur.formula <- kSterbaFormulas[[cur.depvar.colname]]
	  ## Extract actual independent variables (not coefficients) from "cur.formula".
	  cur.indepvars <- all.vars(expr = cur.formula)
	  cur.indepvars <- cur.indepvars[-grep(pattern = "coef|pi", x = cur.indepvars)]
	  ## Assign vector of independent variables.
	  assign(x = paste0(cur.depvar.colname, ".indepvars"),
	         value = cur.indepvars)
	  ## Subset input data frame to the independent variables mentioned in the model formula.
	  cur.input.data.col.subset <- cur.input.data[, cur.indepvars]
	  ## Remove missing values from "cur.input.data.col.subset".
	  cur.input.data.col.subset.na.omitted <- na.omit(object = cur.input.data.col.subset)
	  ## Try to fit model using nls.
	  ## nls.res <- try(expr =
			 ## nls(formula = cur.formula,
			     ## data = cur.input.data.col.subset.na.omitted,
			     ## start = kStartValsVecs[[cur.depvar.colname]][[cur.species.name]],
			     ## na.action = na.omit,
			     ## control = nls.control(maxiter = 1000)),
		       ## silent = TRUE)
	  ## Try to fit model using nls2.
	  ## nls2.res <- try(expr =
			  ## nls2(formula = cur.formula,
			       ## data = cur.input.data.col.subset.na.omitted,
			       ## start = kStartValsVecs[[cur.depvar.colname]][[cur.species.name]],
			       ...
			       ## na.action = na.omit),
	  ## silent = TRUE)
	  ## Try to fit model using nlme.
	  ## for (cur.coef.name in names(x = kStartValsVecs[[cur.depvar.colname]][[cur.species.name]])) {
	      ## assign(x = cur.coef.name,
		   ## value = kStartValsVecs[[cur.depvar.colname]][[cur.species.name]][[cur.coef.name]])
	  ## }
	  ## nlme.res <- try(expr =
			  ## nlme(model = cur.formula,
			       ## fixed = as.formula("gha ~ 1"),
			       ## data = cur.input.data.col.subset.na.omitted,
			       ## start = kStartValsVecs[[cur.depvar.colname]][[cur.species.name]],
			       ## verbose = TRUE))
	  ...
	  assign(x = cur.coef.name,
	         value = kSterbaCoefs[[cur.species.name]][[cur.coef.name]])
        }
        ## Create a vector of test top heights.
        test.h100 <- seq(from = kXLim[1],
		     to = kXLim[2],
		     by = 0.1)
        ## Calculate basal area using Sterba’s Gmax function.
        Sterba.gha.predictions <- 1 / 10000 * pi / (16 * a0 * b0 * test.h100 ^ (a1 + b1))
        ## Add curve of Sterba predictions to plot.
        lines(x = test.h100,
	    y = Sterba.gha.predictions,
	    col = "red",
	    lwd = 1)
    }
    ## Turn off graphics device.
