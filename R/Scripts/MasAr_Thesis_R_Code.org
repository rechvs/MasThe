...
* Preamble
  ...
  #+BEGIN_SRC R :session A
    ...
    ## Set dimensions of PDF device for different device layouts.
    kCmToInch <- 1 / 2.54
    kMmToInch <- 1 / 25.4
...
* Location of sample plots
  ...
  #+BEGIN_SRC R :results silent :var input=Preamble :session A
    ## TEX file: ~/laptop02_MasAr/LaTeX/Thesis/Plot_Location.tex
    ...
    kPdfPointSize <- 90
    kPdfFamily <- "Times"
    kPlotMargins <- c(0, 0, 0, 0)  ## As small as possible using fractions of lines.
    kBoundaryLineWidth <- 4
    kPointCharacter <- 20
    kSpeciesNames <- c("beech", "spruce")
    kPlotLayout <- 1:2
    ## Turn off graphics device.
    ...
    for (cur.plot.nr in kPlotLayout) {
	   ...
	   lwd = kBoundaryLineWidth)
        ...
        if (cur.plot.nr == max(kPlotLayout)) {
