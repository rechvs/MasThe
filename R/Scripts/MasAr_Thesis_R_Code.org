...
* Preamble
  #+NAME: Preamble
  #+BEGIN_SRC R :results silent :session A
...
* log(N)-log(D)-plots before/after data selection
  ...
  #+BEGIN_SRC R :results silent :var input=Preamble :session A
  ...
  #+BEGIN_SRC R :results silent :var input=Preamble :session A
    ...
    for (cur.species.name in kSpeciesNames) {
	  ...
	  for (cur.function.name in kFunctionNames) {
        ...
        cat(cur.output,
	  file = paste0(kOutputSubdir, cur.species.name, "_geographic_properties_summary.txt"),
	  sep = "\n",
	  fill = FALSE)
    }
    ## Clean up workspace.
...
** General procedure:
   1. Calculate column “SI.h100” based on columns “h100” and “alt” using the function by Nagel (1999) (cp. [[file:DataSetCreation.R::Create%20"gmax_merged_1.5.RData"][DataSetCreation.R, block Create "gmax_merged_1.5.RData"]]).
   2. Set “SI.h100” ranges values per yield class, based on “SI.h100” values taken from Schober (1995) (moderate thinning) (cp.  [[file:DataSetCreation.R::Create%20"gmax_merged_5.3.RData"][DataSetCreation.R, block Create "gmax_merged_5.3.RData"]]).
   3. Compare column “SI.h100” from 1. with the “SI.h100” ranges per yield class from 2. and store the comparison result in column “SI.h100.class.new” accordingly (cp.  [[file:DataSetCreation.R::Create%20"gmax_merged_5.3.RData"][DataSetCreation.R, block Create "gmax_merged_5.3.RData"]]).
** R Code
   #+NAME: Yield class classification of training data
   #+BEGIN_SRC R :results silent :var input=Preamble :session A
     ## TEX file: ~/laptop02_MasAr/LaTeX/Thesis/Data_Description.tex
     ## Plotting preamble.
     kPlotMargins <- c(4, 4, 4, 1)
     kPlotLayout <- c(2, 1)
     ...
     pdf(file = file.name,
         ...
         height = kPdfHeightA4Portrait * 0.75,
         pointsize = kPdfPointSize,
         family = kPdfFamily)
     ## Set plot layout.
     ...
     for (cur.species.name in "spruce") {  ## TESTING
         ...
         ## Get x- and y-limits for current plot.
         xlim <- c(0, 160)
         ylim <- c(0, 40)

         ## Set SI.h100 values. Values for yield classes 4, 3, 2, and 1 are taken from Schober (1995) (moderate thinning). Values for yield classes 0, -1, and -2 are linearly interpolated from values for yield classes 2 and 1.
	   ...
	   SI.h100.yield.class.1 <- 35.1
         ...
         ## Calculate SI.h100 value for yield classes 3.5 and -1.5.
         SI.h100.yield.class.3.5 <- SI.h100.yield.class.3 + (SI.h100.yield.class.4 - SI.h100.yield.class.3) / 2
         SI.h100.yield.class..1.5 <- SI.h100.yield.class..1 - (SI.h100.yield.class..1 - SI.h100.yield.class..2) / 2
         ## Generate a sequence ranging from SI.h100 of yield class 3.5 to SI.h100 of yield class -1.5, with 0.1 as the distance between sequence elements.
         SI.h100.sequence <- round(x = seq(from = SI.h100.yield.class.3.5,
				   to = SI.h100.yield.class..1.5,
				   by = 0.1),
			     digits = 1)
	    ...
	    main = cur.species.name,
	    xaxs = "i",
	    yaxs = "i",
	    panel.first = abline(h  = seq(from = ylim[1],
				    to = ylim[2],
				    by = 10),
			     v = seq(from = xlim[1],
				   to = xlim[2],
	   ...
	   lines(x =edvid.subset[["alt"]],
         ...
         ## Loop over all evdids. We use a second loop here in order to make sure that point are drawn after all lines.
         for (cur.edvid in levels(x = bart.cur.species.clean.1.8[["edvid"]])) {
	   ## Create subset of "bart.cur.species.clean.1.8", based on current edvid.
	   edvid.subset <- subset(x = bart.cur.species.clean.1.8,
			      subset = edvid == cur.edvid)
	   ...
	   points(x = edvid.subset[["alt"]],
		...
		pch = kPointCharacter)
         ...
         SI.h100.yield.class.values.color.index <- match(x = round(x = c(
							 SI.h100.yield.class..1,
							 SI.h100.yield.class.0,
							 SI.h100.yield.class.1,
							 SI.h100.yield.class.2,
							 SI.h100.yield.class.3
						       ),
						       digits = 1),
				      table = SI.h100.sequence)
         ## Add legend.
         legend.legend <- c(paste0("yield class ", seq(from = -1, to = 3, by = 1)))
