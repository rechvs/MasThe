...
kFormulas[["Sterba_dgGmax"]] <- as.formula(object = "dg ~ 1 / (a0 * (h100 ^ a1) * nha + b0 * (h100 ^ b1))")  ## cp. Wördehoff (2014), (Gl. 1)
kStartValsGrids[["Sterba_dgGmax"]] <- expand.grid("a0" = c(-2, 10),
                                                  "a1" = c(-2, 10),
                                                  "b0" = c(-2, 10),
                                                  "b1" = c(-2, 10))
kStartValsVecs[["Sterba_dgGmax"]] <- c("a0" = 1 * 10 ^ -6,  ## "stats::nls" converges with these starting values.
                                       "a1" = 0,
                                       "b0" = 2,
                                       "b1" = -2)
## ## Setup for model "Sterba_NGmax".
kFormulas[["Sterba_NGmax"]] <- as.formula(object = "nha ~ b0 / a0 * (2 * b0 * dg) ^ (a1 / b1 -1)")  ## cp. Wördehoff (2014), (Gl. 2)
