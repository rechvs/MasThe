...
yield.table.spruce.schober.1975 <- list(moderate.thinning = list(EKL.I = EKL.I,
                                                                 ...
                                                                 EKL.IV = EKL.IV,
                                                                 EKL.V = EKL.V))
## Calculate mean heights for all EKLs for moderate thinning of spruce not listed in Schober (1975).
age <- EKL.I$age
mean.height <- EKL.I$mean.height
diff.age <- diff(x = age)
...
EKL.I.complete <- data.frame(age = seq(from = age[1],
                                       to = age[length(x = age)]),
