...
* Location of sample plots
  ...
  #+BEGIN_SRC R :results silent :var input=Preamble :session A
    ...
    kPointCharacter <- 20
    kSpeciesNames <- c("beech", "spruce")
    kPointColors <- list("beech" = "#ee7f00",  ## Colors taken from Niedersächsische Landesforsten (2011), fig. 3.
		         "spruce" = "#4066aa")
    kPlotLayout <- 1:2
    ## Turn off graphics device.
    graphics.off()
    ...
    for (cur.plot.nr in kPlotLayout) {
        ...
        cur.species.name <- ifelse(test = cur.plot.nr == 1,
			     ...
			     no = "spruce")
        ## Set point and line color for current species.
        cur.point.line.col <- as.character(x = kPointColors[names(x = kPointColors) == cur.species.name])
        ## Get data frame for current species and plot number.
