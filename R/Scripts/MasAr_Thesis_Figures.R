...
####################################################
## log(N)-log(D)-plot before/after data selection ##
####################################################
...
kLineWidth <- 4
kLineType <- "solid"
kPointCharacters <- list("beech" = 19,
                         "spruce" = 19)
kXLab <- "log(D)"
kYLab <- "log(N)"
kXMin <- 0.5
...
graphics.subdir <- "Graphics/Thesis/"
...
for (cur.species.name in kSpeciesNames) {
    ...
    for (cur.plot.nr in kPlotLayout) {
        ## Set species name for current plot number ("beech" for plot numbers 1 and 2, "spruce" for plot numbers 3 and 4).
        ...
        ## Set point character for current species.
        cur.point.character <- as.numeric(x = kPointCharacters[names(x = kPointCharacters) == cur.species.name])
