...
** CURRENT Exclude measurements from data set if they do not resemble self-thinning
   ...
   + Rewrite block [[file:R/Scripts/DataSetCreation.R::Create%20"gmax_merged_1.9.RData"%20##][Create "gmax_merged_1.9.RData"]] such that it excludes all consecutive measurements if the slope of the log(density)-log(diameter)-line lies outside the aforementioned range.
* NOTES
