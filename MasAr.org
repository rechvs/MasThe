...
**** Data Selection [1/2]
     ...
     + [ ] Report min, mean, and max of slopes of observations which were selected by the data selection mechanism.
...
**** Description of explanatory variables used in the models
     + [ ] Computation of age variable
     + [ ] Computation of site class variable
     + [ ] Figure: Site class variable over stand age (observations classified into yield classes) [Might be a bit redundant when compared to the figure of site index over stand age.]
**** Explanation of GAMs [0/1]
     + [ ] Why did I choose gamma distribution?
**** Explanation of SCAMs [0/2]
     + [ ] Explanation of B-splines (cp. de Boor (2001))
     + [ ] Explanation of P-splines (cp. Eilers & Marx (1996))
**** Explanation of GAMLSSs [0/1]
     + [ ] Explanation of shape constrained P-splines (cp. Boaerts et al. (2006))
*** Discussion
    + Despite their comparatively large geographical distribution, the small number of sample plots did not allow using latitude and longitude as explanatory variables.
**** Show validity of assumed probability distributions via Q-Q-Plots
**** Compare results to Sterba’s approach
**** Discuss data selection mechanism
     + Compare mean of observed slope with slopes reported in literature and point out that the data selection mechanism is rather generous in terms of maximum allowed slopes.
