...
\end{enumerate}
An observation is considered to be a maximum basal area observation and consequently selected if and only if all questions are answered positively.  While being based on literature values (cp. \Cref{tab:SpeciesSpecificReinekeSlopes}), the slope thresholds are essentially arbitrary and a compromise of 2 conflicting goals:  do not select observations of stands which clearly are not subject to self-thinning;  select enough observations to ensure reliable model results.  The slope thresholds are reported in \Cref{tab:ReinekeSlopeThresholds} and form a range of allowed slopes.  For \Beech{} it lies between \num{-2.91} and \num{-0.9}, whereas for \Spruce{} it lies between \num{-2.82} and \num{-0.65}.
