...
\begin{equation}
  ...
  N_{i, j}(t) = \frac{t - t_i}{t_{i + j} - t_i} N_{i, j - 1}(t) + \frac{t_{i + j + 1} - t}{t_{i + j + 1} - t_{i + 1}} N_{i + 1, j - 1}(t) \quad \text{for } j = 1, 2, \ldots, p.
\end{equation}
Then the curve defined by
\begin{equation}
  \label{eq:BSplineCurveDefinition}
  \symbf{C}(t) = \sum_{i = 0}^n \symbf{P}_i N_{i, p}(t)
\end{equation}
is a B-spline \parencite{Weisstein2017a}.  This can then be used for regression by estimating the vector of control points, commonly done via the method of ordinary least squares \parencite{Bollaerts2006}:  for \(q\) observations \((x_j, y_j)\), determine the control points which minimize
\begin{equation}
  \label{eq:BSplineOLS}
  S_B =
  \sum_{j = 1}^q 
  ...
  \Biggr)^2.
\end{equation}
The concept of P-splines adds a penalty to \RefEq{eq:BSplineOLS}.  This penalty is based on high-order (\(k \geq 2\)) finite differences between the coefficients of adjacent B-splines.  The least squares objective function to minimize thus becomes
\begin{equation}
  \label{eq:PSplineOLS}
  S_P =
  \sum_{j = 1}^q 
...
\end{equation}
\parencite{Eilers1996}, with \(\upDelta^k \symbf{P}_i\) being the \(k\)th-order differences, i.e., \(\upDelta^k\symbf{P}_i = \upDelta^1 \bigl(\upDelta^{d - 1} \symbf{P}_i\bigr)\) with \(\upDelta^1 \symbf{P}_i = \symbf{P}_i - \symbf{P}_{i - 1}\) and with \(\lambda\) being a smoothness parameter \parencite{Bollaerts2006}.
